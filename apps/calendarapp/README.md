...
DOCKER_FILE_BUILD_COMMAND=echo yarn workspace @app/calendarapp build
DOCKER_COMPOSE_APP_IMAGE_TAG=calendarapp
DOCKER_COMPOSE_START_APP_COMMAND=yarn workspace @app/calendarapp start
...
docker-compose run app yarn workspace @app/calendarapp makemigrations

# migrate!
docker-compose run app yarn workspace @app/calendarapp migrate

# run dev server!
yarn workspace @app/calendarapp dev
```
